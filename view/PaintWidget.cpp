...
void centerpoint(double &x, double &y, double &z, std::vector<float> data)
{
...
PaintWidget::PaintWidget(std::vector<float> data, const bool & useBinaryBuffers):
  WGLWidget(), initialized_(false), useBinaryBuffers_(useBinaryBuffers), data_(data)
{
...
void PaintWidget::initializeGL()
{
  ...
  // is true.
  bufferDatafv(ARRAY_BUFFER, data_.begin(), data_.end(), STATIC_DRAW, useBinaryBuffers_);
...
void PaintWidget::paintGL()
{
  ...
  drawArrays(TRIANGLES, 0, data_.size()/6);
}
